...
with tab_contacts:
    ...
    # Score composite : CSE(10) + rôle bureau(7)/titulaire(3) + email(5) + phone(3+2mob) + sources(5) + fraîcheur(2) + campagne(+6)
    _SCORE_SQL = f"""
...
""")
    ...
    with sub_mail:
        ...
        _mail_where = ["ql.status != 'merged'", "ql.email IS NOT NULL"]
        if project_filter != "Tous":
            _mail_where.append(f"ql.project = '{project_filter}'")
        if mail_seg == "CSE uniquement":
            _mail_where.append("ql.cse_status = 'oui'")
        elif mail_seg == "Syndiqué uniquement":
            _mail_where.append("ql.union_status = 'oui'")

        _mail_where_sql = " AND ".join(_mail_where)

        df_mail = cached_query(f"""
            SELECT
                ({_SCORE_SQL})                               AS score,
                ql.first_name                                AS prénom,
                ql.last_name                                 AS nom,
            ...
            FROM qualified_leads ql
            JOIN entities e ON ql.entity_id = e.id
            WHERE {_mail_where_sql}
            ORDER BY score DESC, nb_sources DESC
            LIMIT 1000
        """)

        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Leads prêts (email)", len(df_mail))
        if not df_mail.empty:
            m2.metric("CSE=oui",      int((df_mail["cse_status"] == "oui").sum()))
            m3.metric("Aussi tél.",   int(df_mail["phone"].notna().sum()))
            m4.metric("Score moyen",  round(float(df_mail["score"].mean()), 1))

        st.dataframe(df_mail, use_container_width=True, hide_index=True)

        if not df_mail.empty:
            csv = df_mail.to_csv(index=False).encode("utf-8")
    ...
    with sub_call:
        ...
        _call_where = ["ql.status != 'merged'", "ql.phone IS NOT NULL"]
        if project_filter != "Tous":
            _call_where.append(f"ql.project = '{project_filter}'")
        if call_seg == "CSE uniquement":
            _call_where.append("ql.cse_status = 'oui'")
        elif call_seg == "Syndiqué uniquement":
            _call_where.append("ql.union_status = 'oui'")

        _call_where_sql = " AND ".join(_call_where)

        df_call = cached_query(f"""
            SELECT
                ({_SCORE_SQL})                               AS score,
                ql.first_name                                AS prénom,
                ql.last_name                                 AS nom,
            ...
            FROM qualified_leads ql
            JOIN entities e ON ql.entity_id = e.id
            WHERE {_call_where_sql}
            ORDER BY score DESC, nb_sources DESC
            LIMIT 1000
        """)

        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Leads prêts (tél.)", len(df_call))
        if not df_call.empty:
            m2.metric("CSE=oui",      int((df_call["cse_status"] == "oui").sum()))
            m3.metric("Aussi email",  int(df_call["email"].notna().sum()))
            m4.metric("Score moyen",  round(float(df_call["score"].mean()), 1))

        st.dataframe(df_call, use_container_width=True, hide_index=True)

        if not df_call.empty:
            csv = df_call.to_csv(index=False).encode("utf-8")
...
Chaque contact reçoit un score de 0 à 25 :
